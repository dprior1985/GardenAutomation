...
#Test Relay - Disable after test
	...
	sq53 =  "update RunNumber set Water = -6 where Water >= 0 and  RunnumberId in ( select %s from (select distinct RunnumberId from RunNumber where Water >= 1 and timestampdiff(now(),DateNow ) >= 0 and timestampdiff(HOUR,DateNow,NOW()) <= 1 ) e);" %  (int(RunNumber))
	try:
	   # Execute the SQL command
   		cursor.execute(sq53)
	   # Commit your changes in the database
	...
	sq53 =  "update RunNumber set Water = 3 where Water <= 0 and  RunnumberId in ( select %s from (select distinct RunnumberId from RunNumber where Water >= 1 and timestampdiff(now(),DateNow ) >= 0 and timestampdiff(HOUR,DateNow,NOW()) <= 24 ) e);" %  (int(RunNumber))
	try:
	   # Execute the SQL command
   		cursor.execute(sq53)
	   # Commit your changes in the database
